...
class Picking(models.Model):
    ...
    def action_cancel(self):
        if self.state == 'done':
            self.mapped('move_lines')._action_cancel_done()
                            ...
                            for line in move_id.move_line_ids:
                                if line.lot_id:
                                    if line.product_id.tracking == 'lot':
                                        incoming_quant = self.env['stock.quant'].sudo().search([('product_id','=',move.product_id.id),('location_id','=',pack_op_id.location_dest_id.id),('lot_id','=',line.lot_id.id)])
                                        incoming_customer_quant = self.env['stock.quant'].sudo().search([('product_id','=',move.product_id.id),('location_id','=',pack_op_id.location_id.id),('lot_id','=',line.lot_id.id)])
                                        if incoming_quant:
                                        ...
                                        incoming_quant = self.env['stock.quant'].sudo().search([('product_id','=',move.product_id.id),('location_id','=',pack_op_id.location_id.id),('lot_id','=',line.lot_id.id)])
